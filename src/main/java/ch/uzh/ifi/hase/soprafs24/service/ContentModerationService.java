...
public class ContentModerationService {
    ...
    public double checkToxicity(String comment) {
        ...
        try {
            ...
            if (errorCode.equals("400") && errorResponse.getAsJsonObject("error").get("message").getAsString().contains("LANGUAGE_NOT_SUPPORTED_BY_ATTRIBUTE")) {
                log.error("Unsupported language for TOXICITY attribute.");
                return 0.0;
            }
            log.error("Error requesting toxicity check: " + e.getStatusCode() + " " + e.getResponseBodyAsString());
            return 0.0;
        } catch (Exception e) {
            log.error("General error in toxicity check: ", e);
            return 0.0;
        }
    }
    ...
    public double parseSummaryScore(String jsonResponse) {
        try {
            ...
            log.error("Failed to parse the summary score from response", e);
            return -1;
        }
    }
}
